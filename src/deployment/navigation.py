...
def combined_controller(waypoints: np.ndarray, velocities: np.ndarray) -> np.ndarray:
    """
    WAC
...
def image_callback(rgb: Image) -> None:
...
def main(args: argparse.Namespace) -> None:
    ...
    rospy.Subscriber('/rgbd_camera/color/image', Image, image_callback)
    ...
    while not rospy.is_shutdown():
        if len(context_queue) > model_params["context_size"]:
            ...
            if model_params["model_type"] == "MLWAC":
                ...
                with torch.no_grad():
                    # Predict distances
                    dists = model("distance_net", obs_img=current_obs.repeat(len(goal_image), 1, 1, 1),goal_img=goal_image)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Code to run MLWAC navigation on the robot"
    )
    parser.add_argument(
        "--model", "-m",
        default="nomad",
