...
class ViNT_Dataset(Dataset):
    ...
    def _build_caches(self, use_tqdm: bool = True):
        ...
        if not os.path.exists(cache_filename):
            ...
            with lmdb.open(cache_filename, map_size=2**40,max_readers=256) as image_cache:
        ...
        self._image_cache: lmdb.Environment = lmdb.open(cache_filename, readonly=True,max_readers=256)
    ...
    def _after_pad_sequence_pylist(self, sequence, target_length, pad_value=0):
        """
        使用Python列表来填充序列至指定长度，在序列后面进行填充。
        """
        # 计算需要填充的元素数量
        ...
        # 如果需要填充，则在序列后面添加填充
        if padding_size > 0:
            # 假设每个元素都是子列表，创建填充的子列表
            padding = [[pad_value] * len(sequence[0])] * padding_size if sequence else [[pad_value]] * padding_size
            sequence = sequence + padding  # 将填充添加到序列的末尾
        return sequence

    
    
    
    def _before_pad_sequence_pylist(self,sequence, target_length, pad_value=0):
        """
        使用Python列表来填充序列至指定长度。
        """
        # 计算需要填充的元素数量
        # print(len(sequence))
        # print(sequence)
    ...
    def _compute_actions(self, traj_data, curr_time, goal_time):
        ...
        angular = traj_data["angular"][start_index:end_index:self.waypoint_spacing]
        linear = traj_data["linear"][start_index:end_index:self.waypoint_spacing]
        

        if start_index < 5:
            # print(traj_data["actions"][:start_index])
            # print(len(traj_data["actions"][:start_index]))
    # 假设 traj_data["actions"] 和 traj_data["states"] 都是列表的列表
            temp_actions = traj_data["actions"][:start_index+1:self.waypoint_spacing]
            temp_states = traj_data["states"][:start_index+1:self.waypoint_spacing]
            temp_states = self._before_pad_sequence_pylist(temp_states, 6)
            temp_actions = self._before_pad_sequence_pylist(temp_actions, 6)
        else:
            # print(traj_data["actions"][start_index-5:start_index:self.waypoint_spacing])
            temp_actions = traj_data["actions"][start_index-5:start_index+1:self.waypoint_spacing]
            temp_states = traj_data["states"][start_index-5:start_index+1:self.waypoint_spacing]
            temp_states = self._after_pad_sequence_pylist(temp_states, 6)
            temp_actions = self._after_pad_sequence_pylist(temp_actions, 6)
        #读取先前的路航点和yaw序列
        seq_len=5
        if start_index < 5:
            pre_yaw = traj_data["yaw"][:start_index+1:self.waypoint_spacing]
            pre_positions = traj_data["position"][:start_index+1:self.waypoint_spacing]

            # pre_yaw=self._before_pad_sequence_pylist(pre_yaw, 6)
            # pre_positions=self._before_pad_sequence_pylist(pre_positions, 6)
        else:
            pre_yaw = traj_data["yaw"][start_index-5:start_index+1:self.waypoint_spacing]
            pre_positions = traj_data["position"][start_index-5:start_index+1:self.waypoint_spacing]

        if pre_yaw.shape != (seq_len + 1,):
            pre_const_len = seq_len + 1 - pre_yaw.shape[0]
            pre_yaw = np.concatenate([pre_yaw, np.repeat(pre_yaw[-1], pre_const_len)])
            pre_positions = np.concatenate([pre_positions, np.repeat(pre_positions[-1][None], pre_const_len, axis=0)], axis=0)

        assert pre_yaw.shape == (seq_len + 1,), f"{pre_yaw.shape} and {(seq_len + 1,)} should be equal"
        assert pre_positions.shape == (seq_len + 1, 2), f"{pre_positions.shape} and {(seq_len + 1, 2)} should be equal"

        pre_waypoints = to_local_coords(pre_positions, pre_positions[0], yaw[0])
        assert pre_waypoints.shape == (seq_len + 1, 2), f"{pre_waypoints.shape} and {(seq_len + 1, 2)} should be equal"

        if self.learn_angle:
            pre_yaw = pre_yaw[1:] - pre_yaw[0]
            pre_waypoint_yaw = np.concatenate([pre_waypoints[1:], pre_yaw[:, None]], axis=-1)
        else:
            pre_waypoint_yaw = pre_waypoints[1:]
        ...
        # print("pre_waypoint_yaw",pre_waypoint_yaw)
        # print("temp_actions",temp_actions)
        pre_state_matrix = np.hstack((pre_waypoint_yaw, temp_actions[1:]))
        ...
        angular=np.clip(angular[1:], -1, 1)
        linear =np.clip(linear [1:], 0, 1)
        control_commands = np.column_stack((linear, angular))
        if self.normalize:
        ...
        return actions, goal_pos,control_commands,temp_states,pre_state_matrix
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        # Compute actions
        actions, goal_pos,control_commands,temp_states,pre_state_matrix = self._compute_actions(curr_traj_data, curr_time, goal_time)
        pre_state_matrix=torch.as_tensor(pre_state_matrix, dtype=torch.float32)
        # temp_actions=torch.from_numpy(temp_actions).float(),
        # temp_states=torch.from_numpy(temp_states).float()
        # temp_actions=temp_actions[0]
        ...
        return (
            ...
            torch.as_tensor(control_commands, dtype=torch.float32)[:4,1:],
            # torch.from_numpy(temp_actions).float(),
            # torch.from_numpy(temp_states).float()
            # torch.from_numpy(temp_actions).float()[0],
           torch.as_tensor(temp_states, dtype=torch.float32),
           pre_state_matrix
